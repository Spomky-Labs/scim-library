...
abstract class ScimObjectTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertEquals(count($attribute), count($expected), sprintf('At %s, expected keys "%s". Got "%s" (full attribute "%s").', $current_key, json_encode(array_keys($expected)), json_encode(array_keys($attribute)), json_encode($attribute)));
